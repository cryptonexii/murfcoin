...
Check out the source code in the following directory hierarchy.
    ...
    git clone https://github.com/murfcoin-project/gitian.sigs.fluff.git
...
By default, Gitian will fetch source files as needed. To cache them ahead of time, make sure you have checked out the tag you want to build in murfcoin, then:
...
NOTE: Offline builds must use the --url flag to ensure Gitian fetches only from local URLs. For example:
    ...
    pushd ./gitian-builder
    ./bin/gbuild --url murfcoin=/path/to/murfcoin,signature=/path/to/sigs {rest of arguments}
    popd

The gbuild invocations below <b>DO NOT DO THIS</b> by default.

### Build and sign Murfcoin Core for Linux, Windows, and macOS:

    export GITIAN_THREADS=2
    export GITIAN_MEMORY=3000
    
    pushd ./gitian-builder
    ./bin/gbuild --num-make $GITIAN_THREADS --memory $GITIAN_MEMORY --commit murfcoin=v${VERSION} ../murfcoin/contrib/gitian-descriptors/gitian-linux.yml
    ...
    ./bin/gbuild --num-make $GITIAN_THREADS --memory $GITIAN_MEMORY --commit murfcoin=v${VERSION} ../murfcoin/contrib/gitian-descriptors/gitian-win.yml
    ...
    ./bin/gbuild --num-make $GITIAN_THREADS --memory $GITIAN_MEMORY --commit murfcoin=v${VERSION} ../murfcoin/contrib/gitian-descriptors/gitian-osx.yml
...
Build output expected:
  ...
  1. source tarball (`murfcoin-${VERSION}.tar.gz`)
  ...
  4. macOS unsigned installer and dist tarball (`murfcoin-${VERSION}-osx-unsigned.dmg`, `murfcoin-${VERSION}-osx64.tar.gz`)
  5. Gitian signatures (in `gitian.sigs.fluff/${VERSION}-<linux|{win,osx}-unsigned>/(your Gitian key)/`)

### Verify other gitian builders signatures to your own. (Optional)

Add other gitian builders keys to your gpg keyring, and/or refresh keys: See `../murfcoin/contrib/gitian-keys/README.md`.
...
Codesigner only: Sign the macOS binary:
    ...
    transfer murfcoin-osx-unsigned.tar.gz to macOS for signing
    tar xf murfcoin-osx-unsigned.tar.gz
...
Codesigner only: Sign the windows binaries:
    ...
    tar xf murfcoin-win-unsigned.tar.gz
...
```
murfcoin-${VERSION}-aarch64-linux-gnu.tar.gz
murfcoin-${VERSION}-arm-linux-gnueabihf.tar.gz
murfcoin-${VERSION}-i686-pc-linux-gnu.tar.gz
murfcoin-${VERSION}-x86_64-linux-gnu.tar.gz
murfcoin-${VERSION}-osx64.tar.gz
murfcoin-${VERSION}-osx.dmg
murfcoin-${VERSION}.tar.gz
murfcoin-${VERSION}-win32-setup.exe
murfcoin-${VERSION}-win32.zip
murfcoin-${VERSION}-win64-setup.exe
murfcoin-${VERSION}-win64.zip
```
The `*-debug*` files generated by the gitian build contain debug symbols
for troubleshooting by developers. It is assumed that anyone that is interested
in debugging can run gitian to generate the files for themselves. To avoid
end-user confusion about which file to pick, as well as save storage
space *do not upload these to the murfcoin.org server, nor put them in the torrent*.
...
- Upload zips and installers, as well as `SHA256SUMS.asc` from last step, to the murfcoin.org server.
...
- Announce the release:
  ...
  - murfcoin-dev mailing list

  - blog.murfcoin.org blog post

  - Update title of #murfcoin and #murfcoin-dev on Freenode IRC

  - Optionally twitter, reddit /r/Murfcoin, ... but this will usually sort out itself
