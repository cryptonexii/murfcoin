...
#if defined(NDEBUG)
# error "Murfcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x33f08a949c3fc227e9b9cf16176a56fd2377d240c1eaa4a5bf3a9d5e22395878");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Murfcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Murfcoin Signed Message:\n";
...
{
    // Murfcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 205000); // Murfcoin: 10000 blocks in ~4 years

    return nSubsidy + nFees;
}

// Amount of time (in seconds) it takes to for the difficulty to change
static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Murfcoin: Readjust difficulty every day
// Amount of time (in seconds) that a block should be found
static const int64 nTargetSpacing = 60; // Murfcoin: Produce one block every 60 seconds
...
/**
 * Dark Gravity Wave Implementation
 */
...
unsigned int static GetNextWorkRequiredBitcoin(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Murfcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Murfcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x863b119d76361c2f6949037a2386b233def6d42aef2a9e13dcdca4bdb87b0547");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "WP 04/Jan/2018 11:13 AM Dogs are freezing to death";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 2000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0417c6497ff6c671e3da06251fa3b6a8fc99e77c2aa09ee34544efabfa6dce071c8059f3b74bef31ec1bd20567d3bd36662bb0bba0239dd2bad3f440d067f3f6d8") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1515125720;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086413369;

        if (fTestNet)
        {
            block.nTime    = 1515125720;
            block.nNonce   = 386096687;
        }
	...
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xc5c6092184478b2ad8d7d26284aa4d04e4480f83d06ccab56dd6127bb96cfadd"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xf0, 0xc4, 0xd7 }; // Murfcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MurfcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MurfcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MurfcoinMiner(CWallet *pwallet)
{
    printf("MurfcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("murfcoin-miner");
    ...
    try { loop {
        ...
        printf("Running MurfcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MurfcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MurfcoinMiner, pwallet));
}
